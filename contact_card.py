...
def create_contacts(type, number):
    ...
    if type == 'B':
        for i in range(number):
            b_card = BusinessCard(name, surname, phone)
            cards.append(b_card)
    elif type == 'C':
        for i in range(number):
            c_card = ContactCard(name, surname, mail, phone, company, position, phone_work)
            cards.append(c_card)
    return cards
